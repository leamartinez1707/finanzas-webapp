...
- **Balance logic** (`lib/balance.ts`): computes per-member, per-currency balances from expenses + repayments, then a greedy settlement algorithm to minimize the number of payments needed to zero everyone out. Splitting defaults to equal (1/N), but a household can set a default percentage split (`Household.defaultSplit`, frozen into each expense's `splitSnapshot` at creation time) and any single expense can override it with a manual `shares` split — see `expenseShare()`.

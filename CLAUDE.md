...
- **Balance logic**: `lib/balance.ts`'s `computeBalances()`/`expenseShare()` still do the per-member, per-currency math client-side, but only over data that's already fully in memory for a bounded scope — a single month (`BalanceCard`'s "saldo del mes", `/resumen`'s per-month breakdown) — never over "all expenses ever" (that's `householdBalances`, above). `computeSettlements()` (greedy settlement, minimizes the number of payments to zero everyone out) still runs over whichever balance array it's given. Splitting defaults to equal (1/N), but a household can set a default percentage split (`Household.defaultSplit`, frozen into each expense's `splitSnapshot` at creation time) and any single expense can override it with a manual `shares` split — see `expenseShare()`; its SQL twin is `private.expense_share()` (`020_balance_future_dates_and_rounding.sql`), which every server-side aggregate calls — the two implementations must produce **identical** numbers, so a change to one is a change to both. Two invariants they encode: **the payer absorbs the rounding remainder**, so a member's share is never rounded in isolation and the shares always sum to the expense amount exactly (otherwise the members' nets stop summing to zero and each person sees a different debt total); and **nothing dated in the future counts toward "as of now" numbers** — every aggregate that answers "what is owed/available *today*" cuts at `private.today_local()` (Montevideo, not UTC), so a next-month expense only starts counting when its date arrives. Client-side math that gets combined with those server totals (e.g. `BalanceCard`'s "Saldo anterior", which subtracts the current month from the server's all-time net) must apply the same cut, or the subtraction is left comparing two different universes. Per-month *reporting* (`/resumen`, the expense lists, "Gasto compartido del mes") deliberately does **not** cut — navigating to a future month should show that month's expenses.
